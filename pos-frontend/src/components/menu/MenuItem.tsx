...
const MenuItem: React.FC<MenuItemProps> = ({ item }) => {
  ...
  const decrement = () => { if (itemCount > 0) setItemCount((prev) => prev - 1); };
  ...
  return (
    <div className={`glass-card rounded-2xl p-4 min-w-[260px] h-[180px] flex flex-col justify-between transition-all duration-200 hover:shadow-glow hover:-translate-y-0.5 ${!item.isAvailable ? "opacity-40 pointer-events-none" : ""}`}>
      <div className="flex items-start justify-between">
        <h3 className="text-dhaba-text font-semibold text-sm">{item.name}</h3>
        <button onClick={handleAddToCart} className="bg-dhaba-success/15 text-dhaba-success p-2 rounded-xl hover:bg-dhaba-success/25 transition-colors">
      ...
      <div className="flex items-center justify-between gap-2">
        <select
          ...
          onChange={(e) => {
            ...
            if (v) { setSelectedVariant(v); setItemCount(0); }
          }}
        ...
        <div className="glass-input rounded-xl flex items-center gap-4 px-3 py-1.5">
