...
import {
  FaDownload, FaSync, FaTrashAlt, FaDatabase, FaCalendarAlt,
  FaCheckCircle, FaExclamationTriangle, FaFileExport, FaCloudDownloadAlt,
  FaShieldAlt, FaSave,
} from "react-icons/fa";
...
const DataManagement: React.FC = () => {
  ...
  useEffect(() => {
    if (isAdmin) refreshStorageStats();
  }, [isAdmin, refreshStorageStats]);
  ...
  const selectAll = () => {
    setSelectedModules((prev) =>
      prev.length === dataModules.length ? [] : dataModules.map((m) => m.id)
    );
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      const summary = Object.entries(res.data.data).map(([mod, n]) => `${mod}: ${n}`).join(", ");
  ...
  return (
    <section className="bg-dhaba-bg min-h-[calc(100vh-4rem)] pb-8">
            ...
            <div className={`glass-card rounded-3xl p-6 border ${
              ...
              <div className="text-center mb-6">
                <div className={`h-16 w-16 rounded-2xl flex items-center justify-center mx-auto mb-3 ${
                  activeTab === "delete"  ? "bg-dhaba-danger/15"  :
                  activeTab === "sync"    ? "bg-dhaba-info/15"    :
                  activeTab === "backup"  ? "bg-emerald-500/15"   : "bg-dhaba-accent/15"
                }`}>
                ...
                <div className="glass rounded-xl p-4 mb-5 space-y-2">
                  ...
                  {(activeTab === "download" || activeTab === "delete") && (
                    <div className="flex justify-between text-sm">
                      ...
                      <span className="text-dhaba-text font-semibold text-xs">{startDate} → {endDate}</span>
                    </div>
                  )}
                  {activeTab === "download" && (
                    <div className="flex justify-between text-sm">
              ...
              {activeTab === "backup" && (
                <BackupRestoreSection
                  ...
                  startDate={startDate}
                  endDate={endDate}
                  onStorageStatsRefresh={refreshStorageStats}
                />
              )}
      ...
      <DangerZoneSection onStorageStatsRefresh={refreshStorageStats} />
    </section>
  );
};
